# source https://github.com/tguillemot/gaussian_mixture_benchmark
...
def covariances_(estimator):
    if estimator.precision_type == 'full':
        return [linalg.inv(prec) for prec in estimator.precisions_]
    elif estimator.precision_type == 'tied':
...
def plot_data(X, y, estimator):
    ...
    h = plt.subplot(111)
    plt.axis('equal')
    n_components = estimator.means_.shape[0]
