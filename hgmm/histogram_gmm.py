...
# Modified by orisenbazuru <https://github.com/orisenbazuru>
...
def _estimate_gaussian_covariances_full(resp, X, nk, means, reg_covar):
    ...
    Parameters
    ----------
    resp : array-like of shape (n_samples, n_components) weighted by the sample counts

    X : array-like of shape (n_samples, n_features + 1) where last column is count vector
    ...
    X = X[:,:-1] # remove count column
    for k in range(n_components):
        diff = X - means[k]
        covariances[k] = np.dot(resp[:, k] * diff.T, diff) / nk[k]
        # adds small eps (i.e. regularization on the diagonal of the covariance matrix)
...
def _estimate_gaussian_covariances_tied(resp, X, nk, means, reg_covar):
    ...
    """
    count_vec = X[:,-1].reshape(-1,1)
    X = X[:,:-1] # remove count column
    # scale the samples by the count to correctly compute avg_X2
    avg_X2 = np.dot(X.T, X*count_vec)
...
def _estimate_gaussian_parameters(X, resp, reg_covar, covariance_type):
    ...
    """
    # TODO: scale resp matrix using the count vector

    # print('=== _estimate_gaussian_parameters ===')
    # print('X.shape:', X.shape)
    count_vec = X[:, -1].reshape(-1, 1)
    # print('count_vec.shape:', count_vec.shape)
    # print('resp.shape:', resp.shape)

    resp = resp * count_vec # weight the resp matrix with the sample counts
    # print('resp.shape:', resp.shape)
    Xf = X[:,:-1]
...
def _estimate_log_gaussian_prob(X, means, precisions_chol, covariance_type):
    ...
    # print('=== _estimate_log_gaussian_prob ===')
    # count_vec = X[:,-1].reshape(-1,1)
    # print('X.shape:', X.shape)
...
class HistogramGaussianMixture(BaseMixture):
    """Gaussian Mixture.
    ...
    def _check_parameters(self, X):
        ...
        # X has shape (nsamples, nfeatures+1)
    ...
    def _initialize(self, X, resp):
        ...
        Parameters
        ----------
        X : array-like of shape (n_samples, n_features + 1) where last column is the count vector
    ...
    def bic(self, X):
        ...
        Parameters
        ----------
        X : array of shape (n_samples, n_dimensions+1) where last column is the count vector
            The input samples.
        ...
        # count of sample occurence in the dataset
        count_vec = X[:, -1]
        n_samples = count_vec.sum()
        # n_samples = X.shape[0]
        
        return -2 * self.score(X) * n_samples + self._n_parameters() * np.log(
            n_samples
        )
